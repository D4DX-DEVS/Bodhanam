    ...
    <div className="min-h-screen max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:grid lg:grid-cols-[minmax(0,1fr)_20rem] lg:gap-10">
      ...
      <section className="border-b" style={{ borderColor: "var(--border)" }}>
        <div className="py-8 md:py-10">
          ...
          {article.author && (
            <div className="flex items-center gap-3 text-base md:text-lg text-muted mb-6">
              <User size={18} className="flex-shrink-0" />
              <span className="font-sans-ml font-medium">{article.author}</span>
            </div>
          )}
          <Link
            href={`/issue/${article.issueId}`}
            className="inline-flex items-center gap-2 text-sm font-sans-ml font-medium text-primary hover:text-primary-light transition-colors duration-300"
          >
            <ChevronLeft size={16} className="flex-shrink-0" />
            <span>
              Vol {article.issue.volume} · Issue {article.issue.issueNo} · {article.issue.period}
            </span>
          </Link>
        </div>
      </section>
      ...
      {article.coverImage && (
        <section className="border-b" style={{ borderColor: "var(--border)" }}>
          <div className="py-8 md:py-10">
            <div className="relative aspect-video overflow-hidden rounded shadow-sm">
              <img
      ...
      <section className="py-10 md:py-14">
        <div
          className="content-card prose-ml prose-ml-wide text-lg leading-relaxed"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-12 md:mb-16">
            ...
            {adjacent.prev && (
              <Link href={`/articles/show/${adjacent.prev.id}`}>
                <div className="article-card group p-5 md:p-6">
                ...
                {allArticlesInIssue
                  ...
                  .map((a) => (
                    <Link key={a.id} href={`/articles/show/${a.id}`}>
                      <div className="article-card group p-5 md:p-6">
                        {a.category && (
                          <span className="chip text-xs mb-2 block w-fit">{a.category}</span>
                        )}
                        <h3 className="font-serif-ml font-bold text-ink line-clamp-2 group-hover:text-primary transition-colors duration-300 mb-2 leading-tight">
                          {a.title}
                        </h3>
                        {a.author && (
                          <p className="text-xs font-sans-ml text-muted">By {a.author}</p>
                        )}
                      </div>
                    </Link>
                  ))}
